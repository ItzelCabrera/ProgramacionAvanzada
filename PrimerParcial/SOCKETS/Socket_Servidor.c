...
{
	struct sockaddr_in Direccion; /*FUTURO PARAMETRO ESPECIAL PARA HACER EL ENLACE
                                    sockaddr_in PARA SOCKETS DE TIPO INET
                                    sockaddr_un PARA SOCKETS DE TIPO UNIX*/
	struct sockaddr Cliente;
	socklen_t Longitud_Cliente;
	...
	* se abre el socket
	*/
	Descriptor = socket (AF_INET, SOCK_STREAM, 0);//APERUTA EL SOCKET
	if (Descriptor == -1)
	 	return -1;
	...
	*/
	Direccion.sin_family = AF_INET;//DOMICIO -> INET O UNIX
	Direccion.sin_port = Puerto->s_port;//PUERTO
	Direccion.sin_addr.s_addr =INADDR_ANY;//SE ATIENDE A CUALQUIER CLIENTE
	if (bind (
			Descriptor, 
			(struct sockaddr *)&Direccion, 
			sizeof (Direccion)) == -1)
	...
	*/
	if (listen (Descriptor, 1) == -1) //MÁX UN CLIENTE ENCOLADO
	{
		close (Descriptor);
		return -1;
	}
